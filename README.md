...
flowchart TB
    subgraph Client["客戶端"]
        CLI["CLI 指令<br/>app agent/gateway"]
        TG["Telegram 機器人"]
    end
    
    subgraph Core["核心系統"]
        main["main.go<br/>命令分發"]
        agent["Agent 實例<br/>pkg/agent"]
        session["會話管理<br/>pkg/session"]
    end
    
    subgraph LLM["LLM 層"]
        provider["Provider 適配器<br/>pkg/providers"]
        models["OpenAI/DeepSeek<br/>MiniMax/Ollama"]
    end
    
    subgraph Tools["工具層"]
        registry["工具註冊表<br/>pkg/tools"]
        sandbox["沙盒安全<br/>Sandbox"]
        fs["檔案操作<br/>read/write/edit"]
        shell["命令執行<br/>exec"]
        web["網路搜尋<br/>web_search"]
    end
    
    subgraph Workspace["工作區"]
        config["config.json"]
        files["IDENTITY.md<br/>AGENT.md<br/>SOUL.md<br/>USER.md"]
        sessions["sessions/<br/>對話歷史"]
    end
...
flowchart TD
    ...
    build_messages --> compress{"上下文<br/>需要壓縮?"}
...
flowchart LR
    subgraph New["新會話"]
        N1[建立會話管理器] --> N2{session.json<br/>是否存在?}
        N2 -->|否| N3[返回空歷史]
        N3 --> N4[新建會話]
    end
    
    subgraph Exist["已有會話"]
        E1[載入會話管理器] --> E2{session.json<br/>是否存在?}
        E2 -->|是| E3[讀取 JSON 檔案]
        E3 --> E4[反序列化為訊息列表]
    end
...
```mermaid
flowchart TD
    input["使用者輸入"] --> length{長度<br/>> 10000?}
    length -->|是| truncate[截斷到 10000]
    length -->|否| sanitize
    truncate --> sanitize
    
    sanitize --> injection{檢測<br/>注入關鍵字?}
    ...
    escape_bracket --> wrap
    wrap[用 Markdown<br/>code block 包裝] --> output["輸出處理後的輸入"]
```
