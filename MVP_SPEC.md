...
| 項目 | 驗收條件 | 狀態 | 備註 |
|------|----------|------|------|
| **記憶體** | 靜置時 RSS < 10MB | ✅ 通過 | 測試結果：6.21 MB |
| **啟動速度** | 冷啟動 < 1 秒 | ✅ 通過 | 測試結果：41 ms |
| **執行檔大小** | 單一 binary < 15MB | ✅ 通過 | 測試結果：8.1 MB |
| **基本對話** | `app agent -m "What is 2+2?"` 能正確回應 | ⬜ 未測試 | 需要有效的 LLM API Key |
| **工具執行** | Agent 能自主使用工具讀寫檔案、執行指令 | ✅ 通過 | 代碼完整，單元測試通過 |
| **安全沙箱** | 工具無法存取 workspace 以外的路徑 | ✅ 通過 | 8 個單元測試全部通過 |
| **Telegram** | 透過 Telegram Bot 能正常對話 | ⬜ 未測試 | 需要有效的 Bot Token |
| **設定載入** | config.json 能正確載入並支援環境變數覆寫 | ✅ 通過 | 已驗證 |
| **Session** | 對話歷史能持久化並在後續對話中載入 | ✅ 通過 | 代碼完整 |

> **測試日期**：2026-02-23  
> **測試環境**：Windows 11, Go 1.23.5
